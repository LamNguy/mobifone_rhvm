...
class Utils:
	...
	self.cluster_service = self.conn.system_service().clusters_service()    

    def gethosts(self,cluster):
	return [i.name for i in self.hosts_service.list(search='cluster={}'.format(cluster))]

    def listvms( self):
	res = []
	print('List cluster in rhvm')
	clusters = self.cluster_service.list()
	for index , cluster in enumerate(clusters):
		print('{}:{}').format(index,cluster.name)	
	#vms = self.vms_service.list(search='cluster={}'.format(_cluster))
	#for vm in vms:
	#	print(vm.placement_policy.affinity)

    def list_vm (self, rhv_host):

	vms = self.vms_service.list(search=rhv_host)
	res = []
	for vm in vms:
		if (vm.name == 'HostedEngine'): continue

		vm_service = self.vms_service.vm_service(vm.id)
		disk_attachments_service = vm_service.disk_attachments_service()

		#nics_service = vm_service.nics_service()
		#for i in nics_service.list():
		#	vnic_profile =  self.conn.follow_link(i.vnic_profile)
		#	#print(self.conn.follow_link(vnic_profile.network).vlan.id)

		for i in disk_attachments_service.list():
			vmx = {}
			_disk = self.conn.follow_link(i.disk)
			vmx['name'] = vm.name
                        vmx['vcpu'] = vm.cpu.topology.cores*vm.cpu.topology.sockets
                        vmx['ram'] = vm.memory/1024/1024
			vmx['status'] = vm.status	
			
			if _disk.storage_type == types.DiskStorageType.IMAGE:
				vmx['disk_name'] = _disk.alias
				vmx['bootable'] = i.bootable
				vmx['size'] = _disk.provisioned_size/1024/1024/1024
				vmx['disk_id'] = _disk.id
				vmx['lun_id'] = 'None' 
				vmx['disk_type'] = _disk.storage_type
				
			else:
				vmx['disk_name'] = _disk.alias	
				vmx['disk_type'] = _disk.storage_type
				vmx['disk_id'] =  _disk.id
				vmx['lun_id'] = _disk.lun_storage.logical_units[0].id
				vmx['size'] = _disk.lun_storage.logical_units[0].size/1024/1024/1024
				vmx['bootable'] = i.bootable
			res.append(vmx)				
	return res
	...
	vmx = self.vms_service.list(search='name={}'.format(vm.name))	
        ...
        filename = os.getcwd() + '/log/{}'.format(vm.name)
